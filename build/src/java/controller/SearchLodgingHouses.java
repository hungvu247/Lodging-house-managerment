...
public class SearchLodgingHouses extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<LodgingHouse> listSearch;
        if (keyWord.equals("")) {
            listSearch = dao.getAllLodgingHouse();
          
        } else {
            listSearch = dao.searchLodgingHouses(infoSearch);  System.out.println("Loi o day ");
        }

        for (LodgingHouse lodgingHouse : listSearch) {
        ...
        for (LodgingHouse lodgingHouse : pagination.getListObject()) {
            if (lodgingHouse.isStatusDelete()) {
                ...
                out.println("    <a href=\"#\" class=\"btn btn-info btn-sm view-detail\" data-toggle=\"modal\" data-target=\"#detailModal\"\n"
        ...
        if (pagination.getCurentPage() > 1) {
            out.println("<a class=\"pagination-btn\" data-page=\"" + (pagination.getCurentPage() - 1) + "\" value=\"" + (pagination.getCurentPage() - 1) + "\" style=\"padding: 5px 10px; margin-right: 10px; font-size: 20px;cursor:pointer;\">Previous</a>");
        }

        for (int num = pagination.getStart(); num <= pagination.getEnd(); num++) {
            System.out.println("Hello : "+pagination.getEnd());
            if (num != 0) {
                if (pagination.getCurentPage() == num) {
                    out.println("<a class=\"pagination-btn active\" data-page=\"" + num + "\" value=\"" + num + "\" style=\"background-color: green; color: white; border: 1px solid greenyellow; width: 40px; height: 40px;cursor:pointer;; display: inline-flex; justify-content: center; align-items: center; margin-right: 10px; font-size: 20px;\">" + num + "</a>");
                } else {
                    out.println("<a class=\"pagination-btn\" data-page=\"" + num + "\" value=\"" + num + "\" style=\"padding: 5px 10px; margin-right: 10px; font-size: 20px;cursor:pointer;\">" + num + "</a>");
                }
            }
        }

        if (pagination.getCurentPage() < pagination.getNumberOfPage()) {
            out.println("<a class=\"pagination-btn\" data-page=\"" + (pagination.getCurentPage() + 1) + "\" value=\"" + (pagination.getCurentPage() + 1) + "\" style=\"padding: 5px 10px; margin-right: 10px; font-size: 20px;cursor:pointer;\">Next</a>");
        }
        out.println("</div>");
        out.println("</div>");
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        LodgingHousesDAO dao = new LodgingHousesDAO();
        List<LodgingHouse> listSearch;
        if ("".equals(dateMax_raw) && "".equals(dateMax_raw)) {
            listSearch = dao.getAllLodgingHouse();
        } else {
            listSearch = dao.searchLodgingHousesByDate(dateMin_raw, dateMax_raw);
        }
        PrintWriter out = response.getWriter();
        String curentPageRaw = request.getParameter("curentPage");
        int numberPerPage = 6;

    ...
    <body onload="time()" class="app sidebar-mini rtl">
        ...
        <aside class="app-sidebar">
            ...
            <ul class="app-menu">
                ...
                <li><a class="app-menu__item active" href="${pageContext.request.contextPath}/investment-costs-servlet?statusAccept=1"><i
        ...
        <main class="app-content">
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            <c:if test="${sessionScope.account.getRoleId() == 1}">
                                                <th width="100">Loại yêu cầu phê duyệt</th> 
                                    ...
                                    <tbody>                                               
                                        <c:forEach items="${sessionScope.pagination.getListObject()}" var="c">
                                            <tr>
                                                ...
                                                <td>${c.getInformationOfUser().getFullName()}</td>  
                                                <td class="date">
                                                ...
                                                <c:if test="${sessionScope.account.getRoleId() == 1}">
                                                    <td>
                                                        <c:if test="${c.getTypeAccept() == 1}">
                                                            Thêm
                                                        </c:if>
                                                        <c:if test="${c.getTypeAccept() == 2}">
                                                            Sửa
                                                        </c:if>
                                                        <c:if test="${c.getTypeAccept() == 3}">
                                                            Xóa
                                                        </c:if>
                                                    </td>
                                                    <c:if test="${c.getStatusAccept() == 2}">
                                                        <td class="table-td-center" >
                                                            <button class="btn btn-primary btn-sm" type="button" title="Đồng ý" onclick="confirmAccept(${c.getInvestmentCostID()}, 1)">
                                                                <i class="fas fa-check fa-flip" style="--fa-primary-color: #d91c1c; --fa-secondary-color: #d91c1c;"></i>
                                                            </button>
                                                            <button class="btn btn-primary btn-sm" type="button" title="Không đồng ý" onclick="confirmAccept(${c.getInvestmentCostID()}, 3)">
                                                                <i class="fa fa-window-close"></i>
                                                            </button>
                                                        </td>
                                                    </c:if>
                                                    <c:if test="${c.getStatusAccept() == 1}">
                                                        <td class="table-td-center" >
                                                            <button class="btn btn-primary btn-sm">
                                                                <i class="fas fa-check fa-flip btn-sm" ></i>
                                                            </button>
                                                        </td>
                                                    </c:if>

                                                    <c:if test="${c.getStatusAccept() == 3}">
                                                        <td class="table-td-center" >
                                                            <button class="btn btn-primary btn-sm">
                                                                <i class="fa fa-window-close"></i>
                                                            </button>
            ...
            <div class="pagination">
                ...
                <a href="investment-costs-servlet?statusAccept=1"<c:if test="${active == 1}"> class="active" </c:if>>Đã phê duyệt</a>                  
                <a href="investment-costs-servlet?statusAccept=2" <c:if test="${active == 2}"> class="active" </c:if>>Chờ phê duyệt</a>
                <a href="investment-costs-servlet?statusAccept=3" <c:if test="${active == 3}"> class="active" </c:if>>Bị hủy bỏ</a>
            </div>
        </main>
        ...
        <script type="text/javascript">
            function confirmAccept(id, service) {
                if (service === 1) {
                    swal({
                        title: "Cảnh báo",
                        text: "Bạn có chắc chắn là muốn chấp nhận khoản phí đầu tư này?",
                        ...
                        if (willAccept) {
                            swal("Đã chấp nhận thành công.!", {
                                ...
                                window.location = "confirm-investment-costs-servlet?id=" + id + "&service=" + service;
                            });
                        }
                    });
                } else {
                    swal({
                        title: "Cảnh báo",
                        text: "Bạn có chắc chắn là muốn từ chối khoản phí đầu tư này?",
                        buttons: ["Hủy bỏ", "Accept"]
                    }).then((willAccept) => {
                        if (willAccept) {
                            swal("Đã chấp nhận thành công.!", {
            ...
            function deleteRowById(id) {
                swal({
                    ...
                    buttons: ["Hủy bỏ", "Đồng ý"]
                        ...
                        .then((willDelete) => {
                            if (willDelete) {
                                swal("Đã gửi yêu cầu xóa!", {});
                                window.location = "delete-investment-costs-servlet?id=" + id;
                            }
                        });
            }
            jQuery(function () {
